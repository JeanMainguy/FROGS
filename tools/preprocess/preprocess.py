...
if os.getenv('PYTHONPATH') is None:
    ...
    os.environ['PYTHONPATH'] = LIB_DIR + os.pathsep + os.environ['PYTHONPATH']
...
class Pear(Cmd):
    ...
    def __init__(self, in_R1, in_R2, out_prefix, pear_log, param):
        ...
        Cmd.__init__(self,
                     ...
                     + ' --min-overlap ' + str(min_overlap) + ' --max-assembly-length ' + str(
                         max_assembly_length) + ' --min-assembly-length ' + str(min_assembly_length)
...
class Flash(Cmd):
    ...
    def __init__(self, in_R1, in_R2, out_join_prefix, flash_err, param):
        ...
        min_overlap = max(param.R1_size+param.R2_size-param.max_amplicon_size, 10)
        max_expected_overlap = (param.R1_size + param.R2_size) - param.expected_amplicon_size + \
            min(20, int((param.expected_amplicon_size - param.min_amplicon_size)/2))
        out_dir = os.path.dirname(out_join_prefix) if os.path.dirname(
            out_join_prefix) != "" else "."
        Cmd.__init__(self,
                     'flash',
                     'Join overlapping paired reads.',
                     '--threads 1 --allow-outies --min-overlap ' + str(min_overlap) + ' --max-overlap ' + str(max_expected_overlap) + ' --max-mismatch-density ' + str(
                         param.mismatch_rate) + '  --compress ' + in_R1 + ' ' + in_R2 + ' --output-directory ' + out_dir + ' --output-prefix ' + os.path.basename(out_join_prefix) + ' 2> ' + flash_err,
                     ' --version')
...
class Vsearch(Cmd):
    ...
    def __init__(self, in_R1, in_R2, out_prefix, log, param):
        ...
        Cmd.__init__(self,
                     ...
                     + ' --fastqout ' + out_prefix + '.assembled.fastq ' + ' --fastqout_notmerged_fwd ' + out_prefix +
                     '.unassembled_R1.fastq ' + ' --fastqout_notmerged_rev ' + out_prefix + '.unassembled_R2.fastq '
                     + ' --fastq_allowmergestagger --fastq_ascii ' + param.quality_scale +
                     ' --fastq_maxdiffs ' + str(max_diff) +
...
class Remove454prim(Cmd):
    ...
    def __init__(self, in_fastq, out_fastq, cutadapt_log, cutadapt_err, param):
        ...
        Cmd.__init__(self,
                     ...
                     '--five-prim-primer ' + param.five_prim_primer + ' --three-prim-primer ' + param.three_prim_primer + ' --error-rate 0.1 --non-overlap 1 --min-length ' +
                     str(args.min_amplicon_size) + ' -i ' + in_fastq + ' -o ' +
                     out_fastq + ' > ' + cutadapt_log + ' 2> ' + cutadapt_err,
                     '--version')
...
class CutadaptPaired(Cmd):
    ...
    def __init__(self, in_R1_fastq, in_R2_fastq, out_R1_fastq, out_R2_fastq, cutadapt_log, cutadapt_err, param):
        ...
        Cmd.__init__(self,
                     ...
                     '-g \"' + param.five_prim_primer + ';min_overlap=' + str(len(param.five_prim_primer)-1) + '\" -G \"' + revcomp(param.three_prim_primer) + ';min_overlap=' + str(len(
                         param.three_prim_primer)-1) + '\" --error-rate 0.1 --discard-untrimmed --match-read-wildcards --pair-filter=any ' + ' -o ' + out_R1_fastq + ' -p ' + out_R2_fastq + ' ' + in_R1_fastq + ' ' + in_R2_fastq + ' > ' + cutadapt_log + ' 2> ' + cutadapt_err,
                     '--version')
...
class MultiFilter(Cmd):
    ...
    def __init__(self, in_fastq, min_len, max_len, tag, out_fasta, log_file, param):
        ...
        Cmd.__init__(self,
                     ...
                     '--force-fasta --max-N 0' + add_options + ' --input-file ' +
                     in_fastq + ' --output-file ' + out_fasta + ' --log-file ' + log_file,
                     '--version')
    ...
    def parser(self, log_file):
        ...
        if filtered_on_tag is not None:
            FH_log.write('\t(nb seq with expected tag : ' +
                         str(previous_nb_seq - filtered_on_tag) + ')\n')
            previous_nb_seq -= filtered_on_tag
        if filtered_on_length is not None:
            FH_log.write('\tnb seq with expected length : ' +
                         str(previous_nb_seq - filtered_on_length) + '\n')
        ...
        if filtered_on_homopolymer is not None:
            FH_log.write('\tnb seq without large homopolymer : ' +
                         str(previous_nb_seq - filtered_on_homopolymer) + '\n')
            previous_nb_seq -= filtered_on_homopolymer
        if filtered_on_quality is not None:
            FH_log.write('\tnb seq without nearest poor quality : ' +
                         str(previous_nb_seq - filtered_on_quality) + '\n')
...
class Combined(Cmd):
    ...
    def __init__(self, in_R1, in_R2, join_tag, out_join_file):
        ...
        Cmd.__init__(self,
                     ...
                     ' --reads1 ' + in_R1 + ' --reads2 ' + in_R2 + ' -c ' +
                     join_tag + ' --combined-output ' + out_join_file,
                     '--version')
...
class ReplaceJoinTag(Cmd):
    ...
    def __init__(self, combined_input, split_tag, join_tag, out_join_file):
        ...
        Cmd.__init__(self,
                     'combine_and_split.py',
                     'Replace join tag.',
                     ' --reads1 ' + combined_input + ' -s ' + split_tag +
                     ' -c ' + join_tag + ' --combined-output ' + out_join_file,
                     '--version')
...
class DerepGlobalMultiFasta(Cmd):
    ...
    def __init__(self, all_fasta, samples_names, out_samples_ref, out_fasta, out_count, param):
        ...
        Cmd.__init__(self,
                     ...
                     "--nb-cpus " + str(param.nb_cpus) + " --size-separator ';size=' --samples-ref " +
                     out_samples_ref + " --dereplicated-file " + out_fasta + " --count-file " + out_count,
                     '--version')
...
class DerepGlobalFastaCount(Cmd):
    ...
    def __init__(self, fasta, count, out_fasta, out_count, param):
        ...
        Cmd.__init__(self,
                     ...
                     "--nb-cpus " + str(param.nb_cpus) + " --size-separator ';size=' --sequences-file " + fasta +
                     " --samples-count " + count + " --dereplicated-file " + out_fasta + " --count-file " + out_count,
                     '--version')
        ...
        # working through Galaxy
            ...
            if FastqIO.is_valid(file):
                ...
                raise_exception(Exception('\n\n#ERROR :' + file +
                                          ' is neither a fasta or a fastq file\n\n'))
    return out_list
    ...
    for line in FH_count:
        if line.startswith("#id"):
            ...
            if not "FROGS_combined" in seq_id:
                sample_by_seq[seq_id] = {
                    samples_names[idx-1]: int(count) for idx, count in enumerate(line.strip().split()[1:]) if int(count) > 0}
    FH_count.close()
...
def summarise_results(samples_names, lengths_files, log_files, param):
    ...
    for spl_idx, spl_name in enumerate(samples_names):
        ...
        if "artificial combined" in filters:
            ...
            filters_by_sample["artificial combined"][spl_name]["paired-end assembled"] = filters_by_sample["before process"][spl_name] - \
                filters_by_sample["merged"][spl_name]["paired-end assembled"]
        ...
        if file_info.isfile():
            if contiged:
                ...
                if "_R1" in file_info.name or "_r1" in file_info.name:
                    ...
                    raise_exception(Exception("\n\n#ERROR : The file '" + file_info.name + "' in archive '" +
                                              archive + "' is invalid. The files names must contain '_R1' or '_R2'.\n\n"))
        else:
            raise_exception(Exception("\n\n#ERROR : The archive '" +
                                      archive + "' must not contain folders."))
    if len(R1_files) != len(R2_files) and not contiged:
        if len(R1_files) > len(R2_files):
            raise_exception(Exception("\n\n#ERORR : " + str(len(R1_files) - len(R2_files)) + " R2 file(s) are missing in arhive '" +
                                      archive + "'. R1 file : [" + ", ".join(R1_files) + "] ; R2 files : [" + ", ".join(R2_files) + "]\n\n"))
        else:
            raise_exception(Exception("\n\n#ERROR : " + str(len(R2_files) - len(R1_files)) + " R1 file(s) are missing in arhive '" +
                                      archive + "'. R1 file : [" + ", ".join(R1_files) + "] ; R2 files : [" + ", ".join(R2_files) + "]\n\n"))
    try:
        # Extract
...
def filter_process_multiples_files(R1_files, R2_files, samples_names, out_files, out_art_files, lengths_files, log_files, args):
    ...
    for idx in range(len(out_files)):
        if args.already_contiged:
            process_sample(R1_files[idx], None, samples_names[idx],
                           out_files[idx], None, lengths_files[idx], log_files[idx], args)
        else:
            process_sample(R1_files[idx], R2_files[idx], samples_names[idx], out_files[idx],
                           out_art_files[idx], lengths_files[idx], log_files[idx], args)
...
def process_sample(R1_file, R2_file, sample_name, out_file, art_out_file, lengths_file, log_file, args):
    ...
    if not args.already_contiged:
        ...
        uncomb_R1_tmp_cutadapt = tmp_files.add(
            sample_name + '_uncomb_cutadapt_paired_trim_R1.fastq.gz')
        uncomb_R2_tmp_cutadapt = tmp_files.add(
            sample_name + '_uncomb_cutadapt_paired_trim_R2.fastq.gz')
    ...
    try:
        ...
        if not args.already_contiged:
            if args.merge_software == "vsearch":
                vsearch_cmd = Vsearch(R1_file, R2_file, out_contig.replace(
                    ".assembled.fastq", ""), out_contig_log, args)
                vsearch_cmd.submit(log_file)
            elif args.merge_software == "flash":
                flash_cmd = Flash(R1_file, R2_file, out_contig.replace(
                    ".extendedFrags.fastq.gz", ""), out_contig_log, args)
                flash_cmd.submit(log_file)
            elif args.merge_software == "pear":
                pear_cmd = Pear(R1_file, R2_file, out_contig.replace(
                    ".assembled.fastq", ""), out_contig_log, args)
        ...
        if args.sequencer == "454":  # 454
            if is_gzip(out_contig):
                # prevent cutadapt problem (type of file is checked by extension)
                renamed_out_contig = tmp_files.add(sample_name + '_454.fastq.gz')
            else:
                # prevent cutadapt problem (type of file is checked by extension)
                renamed_out_contig = tmp_files.add(sample_name + '_454.fastq')
            ...
            if args.five_prim_primer and args.three_prim_primer:  # Illumina standard sequencing protocol
                ...
                Cutadapt3prim(tmp_cutadapt, out_cutadapt, log_3prim_cutadapt,
                              err_3prim_cutadapt, args).submit(log_file)
            # Custom sequencing primers. The amplicons is full length (Illumina) except PCR primers (it is use as sequencing primers). [Protocol Kozich et al. 2013]
            else:
                out_cutadapt = out_contig

        primers_size = 0
        if args.five_prim_primer is not None:
            primers_size += len(args.five_prim_primer)
        if args.three_prim_primer is not None:
            primers_size += len(args.three_prim_primer)
        min_len = args.min_amplicon_size - primers_size
        max_len = args.max_amplicon_size - primers_size
        # filter on length, N
        MultiFilter(out_cutadapt, min_len, max_len, None, out_NAndLengthfilter,
                    log_NAndLengthfilter, args).submit(log_file)
        ...
        if args.keep_unmerged:
            # remove primers
            if args.five_prim_primer and args.three_prim_primer:  # Illumina standard sequencing protocol
                CutadaptPaired(out_notcombined_R1, out_notcombined_R2, uncomb_R1_tmp_cutadapt,
                               uncomb_R2_tmp_cutadapt, uncomb_log_cutadapt, uncomb_err_cutadapt, args).submit(log_file)
                Combined(uncomb_R1_tmp_cutadapt, uncomb_R2_tmp_cutadapt,
                         "X"*100, art_out_cutadapt).submit(log_file)
            # Custom sequencing primers. The amplicons is full length (Illumina) except PCR primers (it is use as sequencing primers). [Protocol Kozich et al. 2013]
            else:
                Combined(out_notcombined_R1, out_notcombined_R2,
                         "X"*100, art_out_cutadapt).submit(log_file)
            # filter on length, N
            MultiFilter(art_out_cutadapt, args.R1_size, -1, None,
                        art_out_Nfilter, art_log_Nfilter, args).submit(log_file)
            ReplaceJoinTag(art_out_Nfilter, "X"*100, "N"*100, art_out_XtoN).submit(log_file)
            DerepBySample(out_NAndLengthfilter + " " + art_out_XtoN,
                          out_file, out_count).submit(log_file)
        else:
...
def process(args):
    ...
    try:
        ...
        if args.input_archive is not None:  # input is an archive
            samples_from_tar(args.input_archive, args.already_contiged,
                             tmp_files, R1_files, R2_files, samples_names)
            ...
            samples_names = [os.path.basename(current_R1).split('.')[0]
                             for current_R1 in args.input_R1]
        ...
        # Tmp files
        filtered_files = [tmp_files.add(current_sample + '_filtered.fasta')
                          for current_sample in samples_names]
        art_filtered_files = [tmp_files.add(
            current_sample + '_artComb_filtered.fasta') for current_sample in samples_names]
        lengths_files = [tmp_files.add(current_sample + '_lengths.json')
                         for current_sample in samples_names]
        ...
        if nb_processses_used == 1:
            filter_process_multiples_files(
                R1_files, R2_files, samples_names, filtered_files, art_filtered_files, lengths_files, log_files, args)
        else:
            processes = [{'process': None, 'R1_files': [], 'R2_files':[], 'samples_names':[], 'filtered_files':[
            ], 'art_filtered_files':[], 'lengths_files':[], 'log_files':[]} for idx in range(nb_processses_used)]
            # Set processes
        ...
        DerepGlobalMultiFasta(filtered_files, samples_names, tmp_files.add(
            'derep_inputs.tsv'), args.output_dereplicated, args.output_count, args).submit(args.log_file)
...
if __name__ == "__main__":
    ...
    common_arg_parser.add_argument('--min-amplicon-size', type=int, required=True,
                                   help='The minimum size for the amplicons (with primers).')
    common_arg_parser.add_argument('--max-amplicon-size', type=int, required=True,
                                   help='The maximum size for the amplicons (with primers).')
    common_arg_parser.add_argument('--five-prim-primer', type=str,
                                   help="The 5' primer sequence (wildcards are accepted).")
    common_arg_parser.add_argument('--three-prim-primer', type=str,
                                   help="The 3' primer sequence (wildcards are accepted).")
    common_arg_parser.add_argument('--without-primers', action='store_true', default=False,
                                   help="Use this option when you use custom sequencing primers and these primers are the PCR primers. In this case the reads do not contain the PCR primers.")
    common_arg_parser.add_argument('-p', '--nb-cpus', type=int, default=1,
                                   help="The maximum number of CPUs used. [Default: %(default)s]")
    common_arg_parser.add_argument(
        '--debug', default=False, action='store_true', help="Keep temporary files to debug program.")
    ...
    group_common_arg_output.add_argument('-d', '--output-dereplicated', default='preprocess.fasta',
                                         help='FASTA file with unique sequences. Each sequence has an ID ended with the number of initial sequences represented (example : ">a0101;size=10"). [Default: %(default)s]')
    group_common_arg_output.add_argument('-c', '--output-count', default='preprocess_counts.tsv',
                                         help='TSV file with count by sample for each unique sequence (example with 3 samples : "a0101<TAB>5<TAB>8<TAB>0"). [Default: %(default)s]')
    group_common_arg_output.add_argument(
        '-s', '--summary', default='preprocess.html', help='The HTML file containing the graphs. [Default: %(default)s]')
    group_common_arg_output.add_argument('-l', '--log-file', default=sys.stdout,
                                         help='This output file will contain several information on executed commands.')

    # Illumina specific  arguments
    parser_illumina = argparse.ArgumentParser(add_help=False)
    parser_illumina.add_argument('--merge-software', default="vsearch",
                                 choices=["vsearch", "flash", "pear"], help='Software used to merge paired reads')
    parser_illumina.add_argument('--keep-unmerged', default=False, action='store_true',
                                 help='In case of uncontiged paired reads, keep unmerged, and artificially combined them with 100 Ns.')
    parser_illumina.add_argument('--expected-amplicon-size', type=int,
                                 help='The expected size for the majority of the amplicons (with primers), if using Flash as read pair merge software.')
    ...
    parser_illumina.add_argument('--mismatch-rate', type=float, default=0.1,
                                 help='Maxi mismatch rate in overlap region. [Default: %(default)s]')
    parser_illumina.add_argument('--quality-scale', type=str, default="33", choices=[
                                 "33", "64"], help='The phred base quality scale, either 33 or 64 if using Vsearch as read pair merge software [Default: %(default)s]')
    parser_illumina.add_argument('--already-contiged', action='store_true', default=False,
                                 help='The archive contains 1 file by sample : Reads 1 and Reads 2 are already contiged by pair.')

    # Illumina inputs
    group_illumina_input = parser_illumina.add_argument_group('Inputs')
    group_illumina_input.add_argument('--samples-names', type=spl_name_type,
                                      nargs='+', default=None, help='The sample name for each R1/R2-files.')
    group_illumina_input.add_argument(
        '--input-archive', default=None, help='The tar file containing R1 file and R2 file for each sample.')
    group_illumina_input.add_argument(
        '--input-R1', required=None, nargs='+', help='The R1 sequence file for each sample (format: fastq).')
    group_illumina_input.add_argument(
        '--input-R2', required=None, nargs='+', help='The R2 sequence file for each sample (format: fastq).')
...
''')
    ...
    group_454_input.add_argument('--samples-names', type=spl_name_type,
                                 nargs='+', default=None, help='The sample name for each R1/R2-files.')
    group_454_input.add_argument('--input-archive', default=None,
                                 help='The tar file containing R1 file and R2 file for each sample (format: tar).')
    group_454_input.add_argument('--input-R1', required=None, nargs='+',
...
''')
    ...
    Logger.static_write(args.log_file, "## Application\nSoftware: " + os.path.basename(
        sys.argv[0]) + " (version: " + str(__version__) + ")\nCommand: " + " ".join(sys.argv) + "\n\n")
    ...
    if args.input_archive is not None:  # input is an archive
        ...
        if args.input_R1 is None:
            raise_exception(argparse.ArgumentTypeError(
                "\n\n#ERROR : '--R1-files' is required.\n\n"))
        if args.samples_names is not None:
            if len(args.samples_names) != len(args.input_R1):
                raise_exception(argparse.ArgumentTypeError(
            ...
            if len(args.samples_names) != len(set(args.samples_names)):
                duplicated_samples = set(
                    [name for name in args.samples_names if args.samples_names.count(name) > 1])
                raise_exception(argparse.ArgumentTypeError(
                    '\n\n#ERROR : Samples names must be unique (duplicated: "' + '", "'.join(duplicated_samples) + '").\n\n'))
        if args.sequencer == "illumina":
            if not args.already_contiged and args.input_R2 is None:
                raise_exception(argparse.ArgumentTypeError(
                    "\n\n#ERROR : '--R2-files' is required.\n\n"))

    if args.sequencer == "illumina":
        if (args.R1_size is None or args.R2_size is None) and not args.already_contiged:
            raise_exception(
                Exception("\n\n#ERROR : '--R1-size/--R2-size' or '--already-contiged' must be setted.\n\n"))
        if args.without_primers:
            if args.five_prim_primer or args.three_prim_primer:
                raise_exception(argparse.ArgumentTypeError(
                    "\n\n#ERROR : The option '--without-primers' cannot be used with '--five-prim-primer' and '--three-prim-primer'.\n\n"))
        else:
            if args.five_prim_primer is None or args.three_prim_primer is None:
                raise_exception(argparse.ArgumentTypeError(
                    "\n\n#ERROR : '--five-prim-primer/--three-prim-primer' or 'without-primers'  must be setted.\n\n"))
            if args.min_amplicon_size <= (len(args.five_prim_primer) + len(args.three_prim_primer)):
                raise_exception(argparse.ArgumentTypeError("\n\n#ERROR : The minimum length of the amplicon (--min-length) must be superior to the size of the two primers, i.e "+str(
                    len(args.five_prim_primer) + len(args.three_prim_primer)) + "\n\n"))
        if (args.already_contiged and args.keep_unmerged):
            raise_exception(
                Exception("\n\n#ERROR : --already-contiged and keep-unmerged options cannot be used together\n\n"))
        if (not args.already_contiged):
            if args.merge_software == "flash":
                if args.expected_amplicon_size is None:
                    raise_exception(argparse.ArgumentTypeError(
